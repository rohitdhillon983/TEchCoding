...
const Page = () => {
      ...
      const fetchBlogs = async () => {
          const response = await fetch('/api/blogs/getByIdBlogs',{
              method: 'POST',
              headers: {
  ...
  return (
    <div>
        <section className='relative min-h-[80vh] overflow-hidden border w-full flex justify-center bg-[#171616e3]'>
            ...
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-20 text-white text-center w-full flex flex-col items-center">
                <h2 className="text-7xl font-bold w-fit mx-auto">{Blogs[0]?.title}</h2>
                <span className="w-[50%] h-1 bg-[#b91202]"></span>
                <p className="text-2xl mt-2 flex items-center gap-2">Home <FaAngleDoubleRight className='text-[#b91202]'/> Blogs <FaAngleDoubleRight className='text-[#b91202]'/> {Blogs[0]?.title}</p>
            </div>
        </section>
        ...
        <section className="flex min-h-screen justify-between px-6 md:px-16 py-12">
            <div className=" flex flex-wrap gap-7 w-[70%]">
                {Blogs.map((blog) => (
                    <div key={blog._id} className="mb-8 shadow-xl w-full rounded-3xl mx-12 flex flex-col items-center justify-center">
                        <Image src={'https://vatech-html-demo.vercel.app/assets/imgs/pages/blog/blog5.png' || blog.image} alt={blog.title} width={700} height={400} className="mb-4" />
                    
                        <div className="p-4 relative">
                            <h3 className="text-3xl font-bold mb-2">{blog.title}</h3>
                            <p className="text-gray-600 mb-4">{blog.subtitle}</p>
                            <div className="text-gray-600 mb-4" dangerouslySetInnerHTML={{ __html: blog.content }}></div>
            ...
            <div className="w-full md:w-[30%] min-h-screen">
                ...
                <div className="w-full  p-3 border border-gray-300 rounded-3xl mb-4 flex flex-col justify-center">
                    ...
                    {Blogs.slice(0, 3).map((blog) => (
                        ...
                        <Image src={ blog.image || 'https://vatech-html-demo.vercel.app/assets/imgs/pages/blog/blog5.png'} alt={blog.title} width={100} height={100} className="mb-4" />
                        <div className="ml-4">
